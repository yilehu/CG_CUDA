...
int  main()
{
	...
	double *dev_x,*dev_b,*dev_r,*dev_p,*dev_Ax,*dev_PartialSum;
	...
	cudaMemcpy(dev_p,dev_r,n*sizeof(double),cudaMemcpyDeviceToDevice);
	IterationNum = 0;
	...
	while(error>error0)
	{
		...
		error_old = error;
		error = sqrt(error);
		UpdateSearchDirection<<<GridDim,BlockDim>>>(dev_p,dev_r,beta,n);
		END_CLOCK = clock();
